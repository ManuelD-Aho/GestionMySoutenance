...
class CommunicationInterneController extends BaseController
{
    ...
    /**
     * Affiche l'interface de communication interne pour le personnel administratif.
    ...
    {
        $this->requirePermission('TRAIT_PERS_ADMIN_COMMUNICATION_ACCEDER'); // Exiger la permission
        ...
        try {
            ...
            if ($idConversation) {
                // Récupérer les détails de la conversation (méthode à ajouter au service si ce n'est pas déjà fait)
                // Ex: public function getConversationDetails(string $idConversation): ?array
                $currentConversation = $this->messagerieService->getConversationDetails($idConversation);
                ...
                // Ex: public function estParticipant(string $idConversation, string $numeroUtilisateur): bool
                if (!$this->messagerieService->estParticipant($idConversation, $numeroUtilisateur)) {
                ...
                $this->redirect("/dashboard/personnel-admin/communication/{$idConversation}");
                return;
            }

            $data = [
                'page_title' => 'Messagerie Interne Personnel',
    ...
    {
        $this->requirePermission('TRAIT_PERS_ADMIN_COMMUNICATION_ENVOYER_MESSAGE');
        if (!$this->isPostRequest()) {
            $this->redirect("/dashboard/personnel-admin/communication/{$idConversation}");
        }

        $contenuMessage = $this->getRequestData('message_content');
        $rules = ['message_content' => 'required|string|min:1|max:1000'];
    ...
    {
        $this->requirePermission('TRAIT_PERS_ADMIN_COMMUNICATION_CREER_CONVERSATION');

        if ($this->isPostRequest()) {
            $this->handleCreateConversation();
        } else {
            // Afficher un formulaire pour choisir les participants, le nom du groupe, etc.
            // Lister tous les utilisateurs pour le personnel administratif
            $usersList = $this->authService->listerUtilisateursAvecProfils(); // Tous les utilisateurs (étudiants, enseignants, etc.)

            $data = [
                'page_title' => 'Créer une Nouvelle Conversation Interne',
                'users_list' => $usersList,
                'form_action' => '/dashboard/personnel-admin/communication/create'
            ];
            $this->render('PersonnelAdministratif/Communication/create_conversation_form', $data); // Créer cette vue
        }
    }
    ...
    private function handleCreateConversation(): void
    {
        ...
        try {
            ...
            $this->redirect('/dashboard/personnel-admin/communication/create');
        } catch (\Exception $e) {
