...
try {
    ...
    echo "<h1>Erreur Critique de Configuration</h1><p>Impossible de charger le fichier d'environnement.</p>";
    exit;
}
...
session_set_cookie_params([
    ...
    'path' => '/',
    'domain' => getenv('SESSION_DOMAIN') ?: $_SERVER['SERVER_NAME'],
    ...
    switch ($routeInfo[0]) {
        ...
        case FastRoute\Dispatcher::FOUND:
            ...
            if (is_array($handler) && count($handler) === 2 && class_exists($handler[0])) {
                ...
                throw new \RuntimeException("Gestionnaire de route mal configuré ou classe/méthode introuvable pour l'URI: " . htmlspecialchars($uri));
            }
            break;
    }
} catch (\App\Backend\Exception\PermissionException $e) {
    http_response_code(403);
    error_log("Permission Denied: " . $e->getMessage());
    include ROOT_PATH . '/src/Frontend/views/errors/403.php';
} catch (\Throwable $e) {
    http_response_code(500);
    error_log("Erreur non interceptée: " . $e->getMessage() . " dans " . $e->getFile() . ":" . $e->getLine() . "\n" . $e->getTraceAsString());
    if (getenv('APP_ENV') === 'development') {
        echo "<h1>Erreur 500 - Erreur Interne du Serveur</h1>";
        echo "<p><strong>Message:</strong> " . htmlspecialchars($e->getMessage()) . "</p>";
        echo "<p><strong>Fichier:</strong> " . htmlspecialchars($e->getFile()) . " (ligne " . $e->getLine() . ")</p>";
        echo "<h2>Trace de la pile :</h2><pre>" . htmlspecialchars($e->getTraceAsString()) . "</pre>";
    } else {
